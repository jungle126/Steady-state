...
#exit()
#plot1
FigureName = 'FigureS2_a'
fig, ax = plt.subplots(figsize=(3.5, 2),constrained_layout=False)
...
ax.legend(loc='upper right',fontsize=6.5,frameon=False)#,bbox_to_anchor = (1,0.5))
ax.set_title('(a)', fontsize = 9 ,loc = 'left')
ax.set_xlabel('Time of simulation(h)', fontsize=6.5)
ax.set_ylabel(r"Slope of CT distribution", fontsize=6.5)
...
ax.tick_params(axis='y', which='major', direction='in', width=0.8, length=4, labelsize=6.5, pad=2, rotation=0)
ax.tick_params(axis='x', which='major', direction='in', width=0.8, length=4, labelsize=6.5, pad=2, rotation=0)
